...
use builduhc\event\PlayerArenaWinEvent;
...
class Arena implements Listener {
     ...
     */
    public function __construct(BuildUHC $plugin, array $arenaFileData) {
        $this->plugin = $plugin;
    ...
    public function joinToArena(Player $player) {
        if(!$this->data["enabled"]) {
            $player->sendMessage("§c Arena Şu An kuruluyor!");
            return;
        }

        if(count($this->players) >= $this->data["slots"]) {
            $player->sendMessage("§c Arena Dolu!");
            return;
        }

        if($this->inGame($player)) {
            $player->sendMessage("§c Zaten Bir Oyundasın!");
            return;
        }
        ...
        $this->broadcastMessage("§a> {$player->getName()} Adlı Kişi Oyuna Katıldı! §7[".count($this->players)."/{$this->data["slots"]}]");
    }
    ...
    public function disconnectPlayer(Player $player, string $quitMsg = "", bool $death = \false) {
        ...
        if(!$death) {
            $this->broadcastMessage("§7» {$player->getName()} §7Adlı Kişi Oyundan Ayrıldı. §b[".count($this->players)."/{$this->data["slots"]}]");
        }
    ...
    public function startGame() {
        ...
        $this->broadcastMessage("§eOyun Başladı!", self::MSG_TITLE);
    }
    ...
    public function startRestart() {
        ...
        $player->addTitle("§eTebrikler Kazandın!");
       EconomyAPI::getInstance()->addMoney($player, 10); $this->plugin->getServer()->getPluginManager()->callEvent(new PlayerArenaWinEvent($this->plugin, $player, $this));
        $this->plugin->getServer()->broadcastMessage("§7»§b {$player->getName()} §7Adlı Oyuncu§b {$this->level->getFolderName()} §7Adlı Haritayı Kazandı!");
        $player->sendMessage("§a»§bTebrikler Kazandın§a«");
        $player->sendMessage("§6+10 §3Nowa");
        $this->phase = self::PHASE_RESTART;
    }
    ...
    public function onInteract(PlayerInteractEvent $event) {
        ...
        if($this->phase == self::PHASE_GAME) {
            $player->sendMessage("§7» Arena Devam Ediyor");
            return;
        }
        if($this->phase == self::PHASE_RESTART) {
            $player->sendMessage("§7» Arena Yeniden Başlatılıyor!");
            return;
        }
    ...
    public function onLevelChange(EntityLevelChangeEvent $event) {
        ...
        if(!$player instanceof Player) return;
        if($this->inGame($player)) {
            $this->disconnectPlayer($player,"[§bNowa§6LGC] §b> Başarıyla Oyundan Ayrıldın!");
        }
    }
    ...
    public function loadArena(bool $restart = false) {
        if(!$this->data["enabled"]) {
            $this->plugin->getLogger()->error("Arena Yüklenemedi: Arena Aktif Değil");
            return;
        }
